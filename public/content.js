...
(function() {
  ...
  const SCRIPT_ID = 'ai-web-agent-content-script-v3';
  ...
  // ContentDOMUtils Class - ใช้ IIFE เพื่อป้องกันการประกาศซ้ำ
  if (!window.ContentDOMUtils) {
  ...
  // ContentCommandHandler Class - ใช้ IIFE เพื่อป้องกันการประกาศซ้ำ
  if (!window.ContentCommandHandler) {
  ...
  // Chrome Runtime Message Listener - ลบ listener เดิมก่อนเพิ่มใหม่
  if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.onMessage) {
    // ลบ listener เดิม (ถ้ามี)
    if (window.aiWebAgentMessageListener) {
      chrome.runtime.onMessage.removeListener(window.aiWebAgentMessageListener);
    }
    
    // สร้าง listener ใหม่
    window.aiWebAgentMessageListener = (message, sender, sendResponse) => {
    ...
    chrome.runtime.onMessage.addListener(window.aiWebAgentMessageListener);
  }
